...
- ユーザー管理機能
  ...
  - PUT /api/admin/users/:id/status - ユーザーアカウント有効化/無効化
  - GET /api/admin/users/:id - ユーザー詳細
- コンテンツ管理機能
  - GET /api/admin/posts - 投稿一覧・検索
  - DELETE /api/admin/posts/:id - 不適切な投稿削除
  - GET /api/admin/events - イベント一覧・検索
  - DELETE /api/admin/events/:id - イベント削除
  - GET /api/admin/teams - チーム一覧・検索
- 統計情報取得
  - GET /api/admin/stats - ユーザー数、投稿数、イベント数など
- 管理者権限チェックミドルウェア
...
| 管理画面 | 管理者機能、バグ修正 | 40 | Week 10 |
| 最適化 | パフォーマンス、セキュリティ | 40 | Week 11 |
...
### 高リスク項目
1. **想定外のバグ** → バッファ: Week 10で管理画面実装と並行対応
...
- Week 10までに遅延 → 管理画面を簡略化（最低限のユーザー管理のみ）
